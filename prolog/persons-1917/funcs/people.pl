:- module(people, [родственники/2, персоны/1, связи/2, не_описаны/1]).
:- include("../facts/persons.pl").
:- include("../facts/relations.pl").
:- include("../facts/associations.pl").

% ----- Не описаны ----- %

не_описаны(L):-
    не_описаны_в_связях(L1),
    не_описаны_в_объединениях(L2),
    append(L1, L2, L).

% ----- Не описаны в связях ----- %

не_описаны_в_связях(L):-
    добавить_не_описанных_из_связей([], L).

добавить_не_описанных_из_связей(B, L):-
    связь(P1, P2, _),
    (not(персона(P1, _, _, _)), not(member(P1, B)), L1=[P1|B]; not(персона(P2, _, _, _)), not(member(P2, B)), L1=[P2|B]),
    добавить_не_описанных_из_связей(L1, L),!.
...
не_описаны_в_объединениях(L):-
    в_объединениях([P|L1]),
    персоны(L2),
    проверить_объединение(P, L1, L2, [], L),!.

проверить_объединение(P, [P1|L1], L2, B, L):-
    (member(P, L2),проверить_объединение(P1, L1, L2, B, L);
    проверить_объединение(P1, L1, L2, [P|B], L)),!.

проверить_объединение(_, _, _, B, L):-
    L = B,!.
